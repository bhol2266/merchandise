...
const ColorModal = () => {
    ...
    return (
        ...
        <div className={`fixed  transition-transform duration-300 ease-out  z-20  w-[332px] lg:w-[370px] mx-auto  left-0 right-0 ${modalVisible ? 'animate-colorModal' : 'translate-y-[2000px]'}`}>

            <div className={`bg-white w-full  p-[20px] rounded-xl shadow-md`}>

                <h1 className='mb-5 font-inter text-[12px] lg:text-[16px]'>Select Product Colours that you want to publish</h1>
                    ...
                    {colours.map((item, index) => {
                        return (
                            <div onClick={() => { updateFieldChanged(index) }} className={` rounded-lg p-2 border-[1px] ${item.selected ? 'border-[#54BAB9]' : ''}  flex items-center justify-center  space-x-2 cursor-pointer `} key={item.name}>
                                ...
                                <h2 className='font-inter font-medium text-[#323232] text-[10px] lg:text-[12px] '>Preview</h2>
                ...
                <button onClick={() => { setmodalVisible(!modalVisible) }} className={`lg:w-[260px] w-[232px] block mx-auto py-1.5 bg-[#54BAB9] text-[14px] lg:text-[16px] font-inter text-[#FFFFFF] rounded-[4px] cursor-pointer mt-5`}>Confirm</button>
