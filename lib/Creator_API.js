...
const postUserinfo = async (data) => {
    ...
    const rawResponse = await fetch(`${process.env.BACKEND_URL}api/v1/creator/account/userinfo`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
            , 'Authorization': 'Bearer ' + getCookie('accessToken'),
...
const postAccountInfo = async (data) => {

    await refreshTokenfunc()

    const rawResponse = await fetch(`${process.env.BACKEND_URL}api/v1/creator/account/bankdetails`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
            , 'Authorization': 'Bearer ' + getCookie('accessToken'),
...
export {
    refreshTokenfunc, saveYoutubersDetail, uploadSingleImage, publishTshirtsDesign, uploadImageBase64, getYoutubersDetail, getPublishedProducts, putPublishedProducts, deletePublishedProducts, setPublishedProductsPriority, getYoutubersProductsList, getUnitTraking,
    postUserinfo, getUserinfo, postAccountInfo, getAccountInfo
