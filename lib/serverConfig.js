...
const gqlip = `https://45.79.120.101/graphql/`
...
const addPaymentDetails = async (cardId, amount, transactionId, orderId, shippingId) => {
  ...
  var config = {
    method: 'post',
    url: gqlip + `?query=mutation{
      verifyPayment(cartId:${cardId},transactionId:"${transactionId}",orderId:"${orderId}",amount:${amount}.0,shippingId:${shippingId}){
        success
        error
        message
      }
...
const addShippingDetails = async (firstName, lastName, shippingAddress, phoneNum, billingAddress, city, state, pincode, altPhoneNum) => {
  ...
  var config = {
    method: 'post',
    url: gqlip + `?query=mutation{
      addShippingDetails(${billingAddress}:"abc",${shippingAddress}:"dsakdjah",firstName:"${firstName}",lastName:"${lastName}",altPhoneNum:"${altPhoneNum}",city:"${city}",pincode:"${pincode}",phoneNum:"${phoneNum}",state:"${state}"){
        shippingDetails{
          id
        }
      }
...
export {  gqlip, QueryG, token, SignUpUser, SendOTP, VerifyOTP, SignInUser, Addtobag, GetbagItems, UpdatebagItems, addAddress, getAddress, updateAddress, DeletebagItems, applyCoupon, addPaymentDetails, getOrderedItems, addShippingDetails, getShippingDetails }
