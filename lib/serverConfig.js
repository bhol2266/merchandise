...
const addPaymentDetails = async (cardId, amount, transactionId, orderId,shippingId) => {
  ...
  var config = {
    method: 'post',
    url: gqlip + `?query=mutation{
      verifyPayment(cartId:${cardId},amount:${amount},transactionId:"${transactionId}",orderId:"${orderId},shippingId:${shippingId}"){
        success
        error
        message
      }
...
export { apiip, gqlip, QueryG, token, SignUpUser, SendOTP, VerifyOTP, SignInUser, Addtobag, GetbagItems, UpdatebagItems, addAddress, getAddress, updateAddress, DeletebagItems, applyCoupon, addPaymentDetails,getOrderedItems }
